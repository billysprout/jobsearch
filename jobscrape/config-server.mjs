...
//   1. the mcp-jobscrape container (agent-driven configuration: the OpenClaw
//      agent walks a non-technical user through config changes in chat, and
//      applies them here via POST /action), and
//   2. a read-only HTML status page for a browser (GET /).
//
...
//     ?token= — the page is read-only, but still gated.
//
// Write model:
//   - Actions mutate ONLY configs/base.json (content maps: tracks, sources,
//     ats) and configs/production.json (tuning knobs: filters' config,
//     selection) — the exact two layers the 07:00 task loads.
...
function readLayers(configDir) {
  ...
  const prodFile = path.join(configDir, "configs", "production.json");
  return {
    baseFile,
    prodFile,
...
td,th{border:1px solid #ccc;padding:.4rem .6rem;text-align:left}code{background:#f4f4f4;padding:.1rem .3rem}</style></head><body>
...
<p>Configuration is done conversationally — ask the assistant in your usual chat, e.g. <code>block Acme Corp from my job digests</code>.</p>
</body></html>`;
}
