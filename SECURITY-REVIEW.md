...
| 15 | **Multi-tenant/stranger DM abuse** ("someone messaged the bot") | MITIGATED-BY-CONFIG | WhatsApp channel added 2026-08-23 (Option A, WhatsApp leg only — see backlog item #1). `dmPolicy: allowlist` with a single-number `allowFrom` (not the looser default `pairing`), `groupPolicy: disabled`, `sendReadReceipts: false`, `actions.calls: false`, `pluginHooks.messageReceived: false`. Residual risks in §4 R9–R11. | config, cli `channels status --probe` |
